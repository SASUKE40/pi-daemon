...
describe("Cloudflare provisioning", () => {
  ...
  it("creates an exclusive GitHub organization policy and token-file-compatible tunnel", async () => {
    ...
    const fetcher: typeof fetch = async (input, init) => {
      ...
      if (url.endsWith("/access/identity_providers") && !init?.method) return response([{ id: "github-id", name: "GitHub", type: "github" }]);
    ...
    expect(result.config.access).toEqual(githubAccess);
    expect(result.config.allowedEmail).toBeUndefined();
    ...
    expect(body.allowed_idps).toEqual(["github-id"]);
    expect(body.auto_redirect_to_identity).toBe(true);
    expect(body.policies[0].include).toEqual([{ "github-organization": {
      name: "SASUKE40",
      identity_provider_id: "github-id",
      team: "pi-admins",
    } }]);
    expect(body.policies[0].require).toEqual([{ login_method: { id: "github-id" } }]);
  ...
  it("refuses to take over an existing tunnel without a saved managed id", async () => {
    const fetcher: typeof fetch = async (input) => {
      ...
      if (url.endsWith("/access/identity_providers")) return response([{ id: "github-id", name: "GitHub", type: "github" }]);
  ...
  it("migrates its saved exact-email OTP policy to GitHub without an open-policy window", async () => {
    const requests: Array<{ url: string; init?: RequestInit }> = [];
    const fetcher: typeof fetch = async (input, init) => {
      const url = String(input);
      requests.push({ url, ...(init ? { init } : {}) });
      ...
      if (url.endsWith("/access/apps/app-id/policies") && !init?.method) return response([{
        id: "legacy-policy", name: "Pi Daemon exact email", decision: "allow",
        include: [{ email: { email: "only@example.com" } }], require: [{ login_method: { id: "otp-id" } }],
      }]);
      if (url.endsWith("/access/apps/app-id/policies/legacy-policy") && init?.method === "PUT") return response({});
      if (url.endsWith("/access/apps/app-id") && init?.method === "PUT") return response({});
      if (url.endsWith("/configurations") && init?.method === "PUT") return response({});
      if (url.endsWith("/token")) return response("tunnel-token-secret");
      throw new Error(`Unexpected request: ${url}`);
    };
    const legacyConfig: CloudflareConfig = { ...githubConfig, access: undefined, allowedEmail: "only@example.com" };

    const result = await new CloudflareClient("api-token-secret", fetcher).provision({ ...provisionInput(), previous: legacyConfig });

    const policyUpdate = requests.find((item) => item.url.endsWith("/policies/legacy-policy") && item.init?.method === "PUT");
    const appUpdate = requests.find((item) => item.url.endsWith("/access/apps/app-id") && item.init?.method === "PUT");
    expect(JSON.parse(String(policyUpdate?.init?.body)).include[0]["github-organization"]).toMatchObject({ name: "SASUKE40", team: "pi-admins" });
    expect(JSON.parse(String(appUpdate?.init?.body))).toMatchObject({ allowed_idps: ["github-id"], auto_redirect_to_identity: true });
    expect(requests.indexOf(policyUpdate!)).toBeLessThan(requests.indexOf(appUpdate!));
    expect(result.created).toContain("Access policy migrated from email OTP to GitHub");
  });
  ...
  it("refuses an existing Access app with another allow policy", async () => {
    ...
    await expect(new CloudflareClient("api-token-secret", fetcher).provision({ ...provisionInput(), previous: githubConfig }))
      .rejects.toThrow("exclusive GitHub organization policy");
  });
});
